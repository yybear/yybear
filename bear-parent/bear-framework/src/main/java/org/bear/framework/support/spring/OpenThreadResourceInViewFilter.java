...
/**
 * 给当前线程绑定资源，类似spring OpenEntityManagerInViewFilter 和 OpenSessionInViewFilter
 * 的做法. 当然需要绑定的资源是可以配置，相当于XXInViewFilter的一个集合.
 * <p/>
 * 
...
public class OpenThreadResourceInViewFilter extends OncePerRequestFilter {
	...
	protected ThreadResourceManager lookupSessionFactory(
	...
	protected ThreadResourceManager lookupThreadResourceManager() {
		if (logger.isDebugEnabled()) {
			logger.debug("Using ThreadResourceManager '"
					+ getThreadResourceManagerBeanName()
					+ "' for OpenThreadResourceInViewFilter");
		}
		WebApplicationContext wac = WebApplicationContextUtils
				.getRequiredWebApplicationContext(getServletContext());
		return wac.getBean(getThreadResourceManagerBeanName(),
				ThreadResourceManager.class);
	}
}
